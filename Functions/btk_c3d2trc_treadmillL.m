...
treadmill_angle = mean(unwrap(angle2Points(data.marker_data.Markers.lframe_0(:,[1 3]),data.marker_data.Markers.lframe_a2(:,[1 3]))));
% also calculate X and Z translation of origin due to treadmill rotation
x_trans = mean(data.marker_data.Markers.lframe_0(:,1));
z_trans = mean(data.marker_data.Markers.lframe_0(:,3));
...
if length(data.fp_data.GRF_data)>2
    data.fp_data.GRF_data(3:4) = [];
end
...
if isfield(data,'fp_data')
    data = assign_forces(data,{'r_calc','l_calc'},{'calcn_r','calcn_l'},[30 0.25],data.FilterFreq);
end
